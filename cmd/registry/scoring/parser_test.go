...
func TestValidateScoreDefinition(t *testing.T) {
	tests := []struct {
		desc            string
		parent          string
		scoreDefinition *artifacts.ScoreDefinition
		wantNumErr      int
	}{
		// No errors
		{
		...
		},
		{
			...
			scoreDefinition: &artifacts.ScoreDefinition{
				...
				Formula: &artifacts.ScoreDefinition_RollupFormula{
					RollupFormula: &artifacts.RollUpFormula{
		...
		},
		{
			...
			scoreDefinition: &artifacts.ScoreDefinition{
				...
				Type: &artifacts.ScoreDefinition_Percent{
					Percent: &artifacts.PercentType{
						Thresholds: []*artifacts.NumberThreshold{
							{
								Severity: artifacts.Severity_ALERT,
								Range: &artifacts.NumberThreshold_NumberRange{ //error
									Min: 60,
									Max: 100,
								},
							},
		...
		},
		{
			...
			scoreDefinition: &artifacts.ScoreDefinition{
				...
				Type: &artifacts.ScoreDefinition_Boolean{
					Boolean: &artifacts.BooleanType{
						Thresholds: []*artifacts.BooleanThreshold{ //error
							{
								Severity: artifacts.Severity_ALERT,
								Value:    false,
							},
...
func TestValidateScoreFormula(t *testing.T) {
	tests := []struct {
		desc          string
		targetPattern *artifacts.ResourcePattern
		scoreFormula  *artifacts.ScoreFormula
		wantNumErr    int
	}{
		// No errors
...
func TestValidateBooleanThresholds(t *testing.T) {
	tests := []struct {
		desc       string
		thresholds []*artifacts.BooleanThreshold
		wantNumErr int
	}{
		// no errors
		{
...
func TestValidateScoreCardDefinition(t *testing.T) {
	tests := []struct {
		desc                string
		parent              string
		scoreCardDefinition *artifacts.ScoreCardDefinition
		wantNumErr          int
	}{
		// No errors
		{
