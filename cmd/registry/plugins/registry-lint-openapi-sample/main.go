...
func (linter *sampleOpenApiLinterRunner) Run(req *artifacts.LinterRequest) (*artifacts.LinterResponse, error) {
...
func lintFile(specPath string, ruleIds []string) ([]*artifacts.LintProblem, error) {
...
func enforceDescriptionLessThan1000Chars(descriptions *[]*DescriptionField) []*artifacts.LintProblem {
	problems := make([]*artifacts.LintProblem, 0)
	for _, description := range *descriptions {
		if len(description.Description) >= 1000 {
			problems = append(problems, &artifacts.LintProblem{
...
func enforceDescriptionContainsNoTagsRuleId(descriptions *[]*DescriptionField) []*artifacts.LintProblem {
	problems := make([]*artifacts.LintProblem, 0)
	for _, description := range *descriptions {
...
func lintWithRule(node *yaml.Node, ruleId string) ([]*artifacts.LintProblem, error) {
