...
func Command() *cobra.Command {
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if isInt64Artifact(inputs[0]) {
				...
				path, err = exportInt64ToSheet(title, inputs)
				if err != nil {
					log.Fatalf("%s", err.Error())
				}
			...
			if err != nil {
				...
				path, _ = exportVocabularyToSheet(inputs[0].Name, vocabulary)
				...
				path, _ = exportVersionHistoryToSheet(inputNames[0], inputs[0])
				...
				path, _ = exportComplexityToSheet("Complexity", inputs)
				...
				path, _ = exportIndexToSheet(inputs[0].Name, index)
	...
	cmd.PersistentFlags().StringVar(&sheetArtifactName, "as", "", "name of artifact to hold url of exported sheet")
